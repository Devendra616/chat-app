...
module.exports = {
  getAllUsers: async (req, res, next) => {
    try {
      const users = await UserModel.getUsers().catch(next);
      ...
      console.log(
        "🚀 ~ file: users.js ~ line 16 ~ getAllUsers: ~ error",
        error
      );
      next(error);
    }
  ...
  createUser: async (req, res, next) => {
    try {
      ...
      console.log("🚀 ~ file: users.js ~ line 25 ~ createUser: ~ user", user);
  ...
  getUserBySAPId: async (req, res, next) => {
    try {
      ...
      console.log(
        "🚀 ~ file: users.js ~ line 31 ~ getUserBySAPId: ~ error",
        error
      );
      next(error);
    }
  ...
  deleteUserById: async (req, res, next) => {
    try {
      ...
      console.log(
        "🚀 ~ file: users.js ~ line 43 ~ deleteUserById: ~ user",
        user
      );
      ...
      console.log(
        "🚀 ~ file: users.js ~ line 52 ~ deleteUserById: ~ error",
        error
      );
      next(error);
    }
  ...
  createSapId: async (req, res, next) => {
    try {
      ...
      const sapUser = await SapModel.createSapId(
        sapid,
        firstName,
        lastName,
        department,
